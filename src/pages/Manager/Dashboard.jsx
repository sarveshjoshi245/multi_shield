...
import { ShieldAlert, UserCheck, Activity, Zap, Lock, MessageSquare, AlertCircle, ChevronRight, Eye, X } from 'lucide-react';
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }} className="animate-fade-in-up">
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 16 }}>
                <div>
                    ...
                    <p style={{ color: 'var(--text-muted)', fontSize: 14, marginTop: 4 }}>Real-time Privileged User Activity & Risk Analytics</p>
                </div>
                ...
                {[
                    ...
                    <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="glass-card" style={{ padding: 20 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                            ...
                            <span style={{ color: 'var(--text-muted)', fontSize: 12, fontWeight: 500 }}>{m.label}</span>
                        </div>
                            ...
                            {['all', 'normal', 'elevated', 'high', 'critical'].map(f => (
                                <button key={f} onClick={() => setFilterLevel(f)}
                                    style={{
                                        ...
                                        background: filterLevel === f ? 'var(--risk-blue-bg)' : 'var(--bg-card)',
                                        color: filterLevel === f ? '#0284c7' : 'var(--text-muted)'
                                    }}>
                                    {f}
                                </button>
                            ))}
                            ...
                            return (
                                <div key={log.id} onClick={() => setSelectedLog(log)}
                                    style={{
                                        ...
                                        padding: 14, background: log.riskScore > 85 ? 'var(--risk-red-bg)' : 'var(--bg-card-subtle)',
                                        border: `1px solid ${log.riskScore > 85 ? 'rgba(239,68,68,0.15)' : 'var(--border-subtle)'}`,
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1, minWidth: 0 }}>
                                        ...
                                        <div style={{ minWidth: 0 }}>
                                            <p style={{ fontSize: 13, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: 'var(--text-primary)' }}>{log.user} <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>({log.role})</span></p>
                                            <p style={{ fontSize: 11, color: 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{log.action}</p>
                                        </div>
                                    </div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 24, flexShrink: 0 }}>
                                        <div style={{ textAlign: 'right' }}>
                                            <p style={{ fontSize: 9, color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 700, letterSpacing: 1 }}>Score</p>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
                                                <div style={{ width: 60, height: 4, borderRadius: 99, background: 'var(--border-primary)', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                    ...
                    <div className="glass-panel" style={{ padding: 24 }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                            {INCIDENTS.map(inc => (
                                <div key={inc.id} style={{ padding: 12, background: 'var(--bg-card-subtle)', border: '1px solid var(--border-subtle)', borderRadius: 10, display: 'flex', gap: 12 }}>
                                    <div style={{ width: 3, borderRadius: 99, background: inc.severity === 'Critical' ? '#ef4444' : '#f59e0b', flexShrink: 0 }} />
                                    <div>
                                        <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-primary)' }}>{inc.type}</p>
                                        <p style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>{inc.user} · {inc.time}</p>
                    ...
                    <div className="glass-card" style={{ padding: 20, background: 'var(--risk-blue-bg)', border: '1px solid var(--border-active)' }}>
                        <h4 style={{ fontSize: 13, fontWeight: 700, color: '#0284c7', marginBottom: 12 }}>Policy Enforcement Rules</h4>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                            {[
                                { label: '≤ 40', action: 'Allow & Monitor', color: '#10b981' },
                                { label: '40–70', action: 'MFA + Restrict', color: '#f59e0b' },
                                { label: '70–90', action: 'Restrict + Admin', color: '#f97316' },
                                { label: '> 90', action: 'BLOCK + Alert Admin', color: '#ef4444' },
                            ].map((r, i) => (
            ...
            <AnimatePresence>
                {selectedLog && (
                    <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }} onClick={() => setSelectedLog(null)}>
                        <motion.div initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.92 }} className="glass-panel" style={{ maxWidth: 640, width: '100%', padding: 32, position: 'relative', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
                                <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                                    ...
                                    <div>
                                        ...
                                        <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>{selectedLog.role} · {selectedLog.sector} Sector</p>
                                    </div>
                                </div>
                                <button onClick={() => setSelectedLog(null)} style={{ background: 'var(--bg-card)', border: 'none', borderRadius: 8, padding: 8, cursor: 'pointer', color: 'var(--text-muted)' }}>
                                    <X size={18} />
                                </button>
                            </div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
                                <div>
                                    <p style={{ fontSize: 9, textTransform: 'uppercase', letterSpacing: 2, color: 'var(--text-muted)', fontWeight: 700, marginBottom: 8 }}>Activity Details</p>
                                    <div style={{ padding: 14, background: 'var(--bg-card-subtle)', borderRadius: 10, border: '1px solid var(--border-subtle)' }}>
                                        {[
                                            ...
                                            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, padding: '6px 0', borderBottom: i < 4 ? '1px solid var(--border-subtle)' : 'none' }}>
                                                <span style={{ color: 'var(--text-muted)' }}>{k}</span>
                                                <span style={{ fontWeight: 600, fontFamily: k === 'IP Address' || k === 'Risk Score' ? 'monospace' : 'inherit', color: k === 'Risk Score' ? getRiskAction(v).color : 'var(--text-primary)' }}>{v}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                <div>
                                    <p style={{ fontSize: 9, textTransform: 'uppercase', letterSpacing: 2, color: 'var(--text-muted)', fontWeight: 700, marginBottom: 8 }}>ML Explainability</p>
                                    <div style={{ padding: 14, background: 'var(--risk-blue-bg)', borderRadius: 10, border: '1px solid var(--border-active)' }}>
