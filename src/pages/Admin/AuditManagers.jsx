    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }} className="animate-fade-in-up">
            ...
            <div>
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: 14, marginTop: 4 }}>Review and investigate suspicious security incidents across all sectors</p>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 24, alignItems: 'start' }}>
                ...
                <div className="glass-panel" style={{ overflow: 'hidden' }}>
                    <div style={{ padding: '24px 32px', borderBottom: '1px solid var(--border-primary)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <span style={{ fontSize: 11, fontWeight: 700, color: '#f43f5e', background: 'var(--risk-red-bg)', padding: '4px 10px', borderRadius: 6 }}>{logs.filter(l => l.status === 'Open').length} UNRESOLVED</span>
                    </div>
                            ...
                            <thead>
                                <tr style={{ borderBottom: '1px solid var(--border-primary)' }}>
                                    {['User', 'Activity', 'Risk Score', 'Time', 'Status'].map(h => (
                                        <th key={h} style={{ padding: '16px 32px', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 1 }}>{h}</th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
                                {logs.map((log) => (
                                    <tr
                                        ...
                                        style={{
                                            ...
                                            cursor: 'pointer',
                                            background: selectedIncident?.id === log.id ? 'var(--risk-blue-bg)' : 'transparent',
                                            transition: 'all 0.2s'
                                        }}
                                        className="hover-row"
                                    >
                                        <td style={{ padding: '16px 32px' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                                <div style={{ width: 28, height: 28, borderRadius: 99, background: 'var(--bg-card-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                    <User size={14} color="var(--text-muted)" />
                                                </div>
                                                <span style={{ fontSize: 13, fontWeight: 600 }}>{log.user}</span>
                                            </div>
                                        </td>
                                        <td style={{ padding: '16px 32px', fontSize: 13, color: 'var(--text-secondary)' }}>{log.activity}</td>
                                        <td style={{ padding: '16px 32px' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                <div style={{ height: 6, width: 40, background: 'var(--border-primary)', borderRadius: 99, overflow: 'hidden' }}>
                                        ...
                                        <td style={{ padding: '16px 32px', fontSize: 13, color: 'var(--text-muted)' }}>{log.time}</td>
                                        <td style={{ padding: '16px 32px' }}>
                                            <span style={{
                                                fontSize: 10, fontWeight: 700, padding: '4px 8px', borderRadius: 6,
                                                background: log.status === 'Approved' ? 'var(--risk-green-bg)' : log.status === 'Blocked' ? 'var(--risk-red-bg)' : 'var(--risk-yellow-bg)',
                        ...
                        {selectedIncident ? (
                            ...
                            >
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
                                    ...
                                    <button onClick={() => setSelectedIncident(null)} style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}>×</button>
                                </div>

                                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                    <div style={{ padding: 16, background: 'var(--bg-card-subtle)', borderRadius: 12, border: '1px solid var(--border-subtle)' }}>
                                        <p style={{ fontSize: 10, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 1, fontWeight: 700 }}>User Entity</p>
                                        <p style={{ fontSize: 15, fontWeight: 700, marginTop: 4, color: 'var(--text-primary)' }}>{selectedIncident.user}</p>
                                        <div style={{ display: 'flex', gap: 12, marginTop: 12 }}>
                                            <div style={{ flex: 1 }}>
                                                <p style={{ fontSize: 9, color: 'var(--text-muted)', textTransform: 'uppercase' }}>Device/IP</p>
                                                <p style={{ fontSize: 11, fontWeight: 600, marginTop: 2, color: 'var(--text-primary)' }}>{selectedIncident.device}</p>
                                                <p style={{ fontSize: 10, color: '#0ea5e9', fontFamily: 'monospace' }}>{selectedIncident.ip}</p>
                                            </div>
                                            <div style={{ flex: 1 }}>
                                                <p style={{ fontSize: 9, color: 'var(--text-muted)', textTransform: 'uppercase' }}>Timestamp</p>
                                                <p style={{ fontSize: 11, fontWeight: 600, marginTop: 2, color: 'var(--text-primary)' }}>{selectedIncident.time}</p>
                                            </div>
                                        </div>
                                    </div>

                                    <div>
                                        <label style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 700 }}>Detected Behavior</label>
                                        <div style={{ padding: 12, background: 'var(--risk-red-bg)', borderRadius: 8, border: '1px solid rgba(239,68,68,0.12)', marginTop: 8 }}>
                                            <p style={{ fontSize: 13, color: '#ef4444', fontWeight: 600 }}>{selectedIncident.behavior}</p>
                                        </div>
                                    </div>

                                    <div>
                                        <label style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 700 }}>Automated Explanation</label>
                                        <p style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 8, lineHeight: 1.6 }}>{selectedIncident.explanation}</p>
                                    </div>

                                    <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
                                        <p style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 700, marginBottom: 4 }}>Admin Actions</p>
                            ...
                            <div className="glass-panel" style={{ padding: 40, textAlign: 'center', borderStyle: 'dashed' }}>
                                <Shield size={40} color="var(--text-faint)" style={{ margin: '0 auto 16px' }} />
                                <p style={{ color: 'var(--text-muted)', fontSize: 14 }}>Select an incident log to review details and take action</p>
                            </div>
                        )}
                    </AnimatePresence>
                </div>
            ...
            <style>{`.hover-row:hover { background: var(--bg-card-hover) !important; }`}</style>
        </div>
    );
}
